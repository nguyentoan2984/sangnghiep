...
---Bước 3: Load danh sách quầy dịch vụ của đơn vị ---> Combo quầy
BEGIN
SELECT 		COUNTERID AS 'Quầy'
...
WHERE		dbo.STAFFS.COUNTERID='1'     ---'BIEN QUAY'  ---THAY LAN LUOT MA QUAY PHUC VU O LIST TREN
...
FROM    	dbo.CUSTOMERS 
WHERE		dbo.CUSTOMERS.STATUS = '1' AND dbo.CUSTOMERS.COUNTERID = '1'  ---'BIEN QUAY'	   ---HIEN THI SO DANG PHUC  VU O QUAY 1
...
BEGIN 
SELECT      DATEDIFF("SECOND",dbo.CUSTOMERS.SERVINGTIME, '2018-10-12 08:59:00')   ----GETDATE() ngay gio hien tai (he thong)
FROM		dbo.CUSTOMERS 
WHERE		dbo.CUSTOMERS.CUSTOMERNO ='1004'     ----'SO DANG PHUC VU'					 ---LA SO DANG PHUC VU CUA CUSTOMER O DICH VU TREN
			AND CONVERT(CHAR(10), TOCOUNTERTIME, 103) = '12/10/2018'  ----'NGAY HIEN TAI dd/mm/yyyy'
END
...
FROM    	dbo.CUSTOMERS
WHERE   	dbo.CUSTOMERS.STATUS = '3' AND dbo.CUSTOMERS.COUNTERID ='1' AND CONVERT(CHAR(10), TOCOUNTERTIME, 103) = '12/10/2018'  ---'NGAY HIEN TAI dd/mm/yyyy'
END
...
BEGIN
SELECT		
			ROUND(AVG(DATEDIFF("SECOND", dbo.CUSTOMERS.SERVINGTIME, dbo.CUSTOMERS.FINISHTIME)), 0) AS AVGSERVETIME,		---//Thoi gian phuc vu trung binh  cua QUAY 
			-----THOI GIAN PHUC VU TRUNG BINH
            ROUND(AVG(DATEDIFF("SECOND", dbo.CUSTOMERS.TOCOUNTERTIME, dbo.CUSTOMERS.SERVINGTIME)), 0) AS AVGWAITTIME,	---//Thoi gian cho trung bình cua QUAY 
			-----CHO HIEN THI VO COT THOI GIAN CHO TRUNG BINH
			MAX(DATEDIFF("SECOND", dbo.CUSTOMERS.SERVINGTIME, dbo.CUSTOMERS.FINISHTIME)) AS MAXSERVETIME				    ---//Thoi gian phuc vu lau nhat cua mot quay 
FROM    	dbo.CUSTOMERS
WHERE   	dbo.CUSTOMERS.STATUS = '3' AND dbo.CUSTOMERS.COUNTERID ='1' AND CONVERT(CHAR(10), TOCOUNTERTIME, 103) = '12/10/2018' -----   'NGAY HIEN TAI dd/mm/yyyy'
END
...
BEGIN
SELECT		DATEDIFF("SECOND", dbo.CUSTOMERS.FINISHTIME,'2018-10-12 08:59:00') AS 'Thời gian nghỉ'   ---- THAY BANG GETDATE() ngay gio hien tai (he thong)
FROM    	dbo.CUSTOMERS
WHERE		dbo.CUSTOMERS.CUSTOMERNO='1004' AND CONVERT(CHAR(10), TOCOUNTERTIME, 103) = '12/10/2018'  ----'NGAY HIEN TAI dd/mm/yyyy'   
-------USTOMERS.CUSTOMERNO LAY MA CUSTOMERNO DA TIM DC O TREN
END
...
---- SO SANH "WAITTIMEMAX" > "AVGWAITTIME"(DOI SANG DON VI LA GIAY, GIA TRI HIEN TAI LA PHUT) ---> TO VANG CELL ---> NEU CO CHECK "Tim theo phuc vu vuot gio han"
---- SO SANH "THOI GIAN PHUC VU HIEN TAI " > SERVICETIMEMAX (DOI SANG DON VI LA GIAY) ---> TO XANH CELL ----> NEU CO CHECK "Tim theo thoi gian cho lau nhat.."
